...
const FAQItem = ({
  ...
  return (
    <div
      className="border-[#D4D4D4] border-[1px] z-50 bg-white/20 backdrop-blur-[36px] p-[24px] rounded-[16px] border-dashed hover:border-[#4F46E5] cursor-pointer transition-colors duration-300"
      onClick={() => setIsOpen(!isOpen)}
    >
      <div className="flex flex-row items-center gap-[16px]">
        <div className="xl:w-[732px] lg:w-[758px] md:w-[612px] sm:w-[259px]">
          <p className="text-[#525252] text-start font-[Archivo] md:text-[20px] sm:text-[18px] leading-[120%] tracking-[-1px] font-[500]">
            {question}
          </p>
        </div>

        <div className="text-[#4F46E5] text-[25px]">
          {isOpen ? (
            <Icons IconPath={"/Assets/Minus.svg"} className="w-[20px] h-[20px]" />
          ) : (
            <Icons IconPath={"/Assets/plus.svg"} className="w-[20px] h-[20px]" />
          )}
        </div>
      </div>
    ...
    <div className="relative bg-white">
      <div className="relative flex flex-col gap-[24px]">
        ...
        <Icons
          ...
          className="absolute bottom-[-75px] z-0 left-[780px] sm:hidden md:hidden lg:flex xl:flex"
        />
      </div>
    </div>
  );
